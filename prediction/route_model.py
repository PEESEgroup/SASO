...
class SelfAttentionSubNet(nn.Module):
    ...
    def forward(self, x):
        ...
        attn_output, _ = self.attn(x, x, x)  # 自注意力输出
        x = attn_output.squeeze(1)  # 移除序列维度
...
class WeightedFusion(nn.Module):
    def __init__(self, input_dim, output_dim):
        ...
        self.weights = nn.Parameter(torch.ones(input_dim))  # 初始化融合权重
        self.fc = nn.Linear(input_dim, output_dim)

    def forward(self, *inputs):
        # 加权组合子网络输出
        weighted_inputs = [w * x for w, x in zip(self.weights, inputs)]
        fused_output = torch.cat(weighted_inputs, dim=-1)  # 拼接所有子网络输出
